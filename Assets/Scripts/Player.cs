...
public class Player : MonoBehaviour
{
    ...
    [SerializeField]
    private GameObject chargeAttackVisual;
    ...
    private void FixedUpdate()
    {
        ...
        Vector3 chargeAttackDir = new Vector3(
            chargeAttackInputVector.x,
            0f,
        ...
        if (chargeAttackDir.magnitude > 0.9)
        {
            ...
            if (chargeAttackCharge < chargeAttackMaxCharge)
            {
                ...
                OnChargeUpdate?.Invoke(
                    this,
                    new OnChargeUpdateEventArgs { charge = chargeAttackCharge }
                );
            }
        }
        else
    ...
    {
        if (collision.collider.GetComponent<IEnemy>() != null)
            TakeDamage(collision.collider.GetComponent<IEnemy>());
    }
    ...
    private void TakeDamage(IEnemy enemy)
    {
        if (timeSinceDamage > invulnerabilityTime)
        {
            ...
            OnHealthUpdate?.Invoke(
                this,
                new OnHealthUpdateEventArgs
                {
                    playerCurrentHealthNormalized = playerHealth / playerMaxHealth
                }
            );
            timeSinceDamage = 0;
            if (playerHealth <= 0)
    ...
    private void Attack(Vector3 aimDir)
    {
        if (timeSinceAttack > attackSpeed)
        {
            ...
            OnShoot?.Invoke(
                ...
                {
                    shooterPos = transform.position + aimDir,
                    shootingDir = aimDir,
                    damage = shootDamage
                }
            );
    ...
    private void ChargeAttack(Vector3 chargeDir)
    {
        ...
        OnShootChargeAttack?.Invoke(
            ...
            new OnShootEventArgs
            {
                shooterPos = transform.position + chargeDir,
                shootingDir = chargeDir,
                damage = chargeAttackCharge
            }
        );
    }
